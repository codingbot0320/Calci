...
class Calculator:
    def __init__(self, master):
        ...
        self.entry_value = ' '
        Entry(master, textvariable=self.equation, width=17, bg='#fff', font=('Arial Bold', 28)).place(x=0, y=0)
        
        Button(width=11, height=4, text="(", relief="flat", bg="white", command=lambda: self.show("(")).place(x=0, y=50)
        Button(width=11, height=4, text=")", relief="flat", bg="white", command=lambda: self.show(")")).place(x=90, y=50)
        Button(width=11, height=4, text='%', relief="flat", bg="white", command=lambda: self.show("%")).place(x=180, y=50)

        Button(width=11, height=4, text='1', relief='flat', bg='white', command=lambda: self.show(1)).place(x=0, y=139)
        ...
        Button(width=11, height=4, text="C", relief='flat', command=self.clear).place(x=0, y=350)
                ...
                # Corrected Button creation and added command for button click
        Button(master, width=11, height=4, text=' ', relief='flat', bg='white', command=lambda: self.show(' ')).place(x=0, y=100)
...
# Corrected the instance creation of Tk
root = Tk()
